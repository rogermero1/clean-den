...
public class JSFUtils {
	...
	public static <T> T findManagedBean(String managedBeanName,
			Class<T> beanClass) {
		return beanClass.cast(getApplication()
				.evaluateExpressionGet(getFacesContext(),
						"#" + "{" + managedBeanName + "}", beanClass));
	}
	...
	public static FacesMessage crearMensajeInformacion(
			String pMensajeInformacion, String pDetalle) {
	...
	public static void removeRequestValue(Object pValue) {
		removeValueFromMap(pValue, getFacesContext().getExternalContext()
				.getRequestMap());
	}
	...
	public static void setNombreArchivoDescargar(String pNombreArchivo) {
		getHttpServletResponse().setHeader("Content-Disposition",
				"inline; filename=\"" + pNombreArchivo + "\"");
	}
